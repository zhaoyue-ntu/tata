...
def get_ds(tree_transform, ds_info, roots, labels):
    trans_roots = tree_transform.transform(roots)
    costs = list(ds_info.cost_norm.normalize_labels(labels))
    return list(zip(trans_roots, costs))

def index_loader(left_ds, right_ds, args):
    left_ld = torch.utils.data.DataLoader(left_ds, batch_size = args.bs, shuffle=False, collate_fn= collate)
    left_batch = next(iter(left_ld))[0].to(args.device)    
    right_ld = torch.utils.data.DataLoader(right_ds, batch_size = args.bs, shuffle=False, collate_fn= collate)
    right_batch = next(iter(right_ld))[0].to(args.device)
    
    return left_batch, right_batch

def construct_loader(ds_info, tree_transform):
    def get_loader(roots, costs, bs = None, shuffle=False):
        ds = get_ds(tree_transform, ds_info, roots, costs)
        if bs is None:
            bs = len(roots)
...
def construct_seeded_loader(ds_info, tree_transform):
    def get_loader(roots, costs, sampler, bs = None,):
        ds = get_ds(tree_transform, ds_info, roots, costs)
        if bs is None:
            bs = len(roots)
        return torch.utils.data.DataLoader(
...
class StatExtractor:
    ...
    def __call__(self, inp):
        res = []
        for f, lo, hi in zip([inp.cost_est, inp.card_est], self.mins, self.maxs):
            if not f:
                res.append(0)
            else:
...
class TreeFeaturizer:
    ...
    def fit(self, trees):
        ...
        self.tree_builder = TreeBuilder(stats_extractor, all_rels)

    def fit_from_ds_info(self, ds_info):
        all_rels = ds_info.tables 
        fields = ['Total Cost', 'Plan Rows']
        mins = [ds_info.cost_est_norm.mini, ds_info.card_norm.mini]
        maxs = [ds_info.cost_est_norm.maxi, ds_info.card_norm.maxi]
        se = StatExtractor(fields, mins, maxs)
        self.tree_builder = TreeBuilder(se, all_rels)

    def transform(self, trees):
        for t in trees:
            _attach_buf_data(t)
        return [self.tree_builder.plan_to_feature_tree(x) for x in trees]
